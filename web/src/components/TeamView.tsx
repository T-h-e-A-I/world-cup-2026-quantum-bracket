...
export default function TeamView({ initialId }: { initialId: number }) {
  ...
  useEffect(() => setId(initialId), [initialId]); // keep in sync when landing on a /team/[slug]

  const onChange = (next: number) => {
    setId(next);
    router.replace(`/team/${slug(next)}`, { scroll: false }); // shareable + SEO-linked
  };
    ...
    <div className="space-y-12">
      ...
      <section>
        <div className="max-w-sm">
          <TeamSelect value={id} onChange={onChange} label="Pick your team" />
        </div>
